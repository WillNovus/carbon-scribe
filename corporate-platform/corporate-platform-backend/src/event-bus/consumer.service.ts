...
export class ConsumerService implements OnModuleDestroy {
        ...
        await consumer.run({
            ...
            eachMessage: async ({ topic, partition, message, heartbeat }) => {
                ...
                try {
                    ...
                    await this.dlqService.routeToDlq(topic, message.value?.toString() || 'empty', error);
                ...
                while (retries <= maxRetries && !success) {
                    try {
                        ...
                        retries++;
                        this.logger.warn(`Failed to process event ${event.id}. Attempt ${retries} of ${maxRetries + 1}`);

                        if (retries > maxRetries) {
                            this.logger.error(`Exceeded max retries for event ${event.id}. Routing to DLQ.`);
    ...
    private async commitOffset(consumer: Consumer, topic: string, partition: number, offset: string) {
