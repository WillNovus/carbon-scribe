...
describe('ConsumerService', () => {
    ...
    describe('consume', () => {
        it('should connect, subscribe, and run the consumer', async () => {
            ...
            expect(mockConsumer.subscribe).toHaveBeenCalledWith({ topic: 'topic1', fromBeginning: false });
        ...
        it('should handle incoming messages successfully and use idempotency', async () => {
            ...
            expect(cacheService.get).toHaveBeenCalledWith('event_bus:processed:event-1');
            expect(handler).toHaveBeenCalledWith(event);
            expect(cacheService.set).toHaveBeenCalledWith('event_bus:processed:event-1', true, 86400);
        ...
        it('should route to DLQ after max retries', async () => {
            ...
            expect(dlqService.routeToDlq).toHaveBeenCalledWith('topic1', event, expect.any(Error));
            expect(mockConsumer.commitOffsets).toHaveBeenCalledWith([
